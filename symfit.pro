...
function get_profile, f_index, s_index, cal_flag
	...
	where_remain = where(remain eq 1)
	if where_remain[0] ne -1 then begin
		num_remain = n_elements(where_remain)
	endif else begin
		num_remain = 1
	endelse
...
pro symfit
	...
	for i = 0, num_sym-1 do begin
					...
					psym = -16, symsize = 0.5, thick = 8, xticks = 4, xminor = 4, $
						xrange = [-0.8, 0.8], xtitle = '!17 Distance to skeleton (pc)', $
							ytitle = '!17 N!DH2!N(r)/N!DH2!N(0)', $
								;yrange = [0,1.3]
								yrange = [0, (max(y_profile[where(y_profile ge 0.)]))*1.2 + $
		...
		cgplot, [terminal[0]*delta, terminal[0]*delta], $
			[0, (max(y_profile[where(y_profile ge 0.)]))*1.2 + max(y_error)], $
...
;---EVALUATION PROCESS OF GOODNESS OF GASSIAN FITTING ...
		...
		printf, lun3, strtrim(string(sym_ind[i]), 2), string(sym_seg[i]), $
			string(psquared, format = '(f11.2)'), string(gsquared, format = '(f11.2)'), $
				string(slice_num, format = '(I)')
		;cgtext, 0.65, 0.82, textoidl('\chi') + '!U2!N!Dp!N = ' $
		;	+ strtrim(string(re_p, format = '(f11.2)'), 2), /normal, color = 'orange_red'
		;cgtext, 0.65, 0.75, textoidl('\chi') + '!U2!N!Dg!N = ' $
		;	+ strtrim(string(re_g, format = '(f11.2)'), 2), /normal, color = 'green'
		cgtext, 0.75, 0.62, textoidl('\chi') + '!U2!N!Dp!N = ' $
			+ strtrim(string(psquared, format = '(f11.2)'), 2), /normal, color = 'orange_red'
		cgtext, 0.75, 0.55, textoidl('\chi') + '!U2!N!Dg!N = ' $
			+ strtrim(string(gsquared, format = '(f11.2)'), 2), /normal, color = 'green'
		num_re = alldata[4, 0]
		count[fix(num_re-1)] = count[fix(num_re-1)] + 1.
		cgtext, 0.65, 0.85, 'slices: ' + $
			strtrim(string(fix(max(num_re))), 2), /normal, color = 'black'
		dir_overlap = '../overlap/in_' + strtrim(string(sym_ind[i]), 2) + '.cat'
		readcol, dir_overlap, o_ind, o_seg, ratio, format = 'I, I, (f11.2)'
		cgtext, 0.1, 0.9, 'P!D' + textoidl('\cap/\cup, max') + '!N = '$
		 + strtrim(string(ratio[sym_seg[i]], format = '(f11.2)'), 2), /normal, color = 'black'
	;endif
	endfor
	print, count
	free_lun, lun
